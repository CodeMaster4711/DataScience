# -*- coding: utf-8 -*-
"""
V6a - Correct Initialization + Weights & Biases Integration
Verbesserung von V5: Korrekte Init + vollständiges Experiment Tracking

V6a Strategie (MIT korrekter Initialisierung):
1. ✅ Korrekte Final Layer Init → Loss @ init = -log(1/10) ≈ 2.3026
2. ✅ Initialization Sanity Check (verify loss @ init)
...
Erwartete Verbesserung: Schnellere initiale Convergence, bessere Gradient Flow
"""
...
class Config:
    ...
    # V6a Specific
    correct_init = True  # v6a: TRUE (correct initialization)
    version = "v6a"
...
class DeeperCNN(nn.Module):
    ...
    def _init_weights(self):
        """
        V6a: CORRECT INITIALIZATION for -log(1/n_classes) initial loss

        For balanced 10-class classification:
        - Target: Initial loss = -log(1/10) = log(10) ≈ 2.3026
        - Strategy: Initialize final layer with small weights and zero bias
                   This gives uniform softmax output → log(10) loss
        """
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                # V6a: KEY CHANGE - Small weight init for correct initial loss
                nn.init.normal_(m.weight, 0, 0.01)  # Small std for near-zero logits
                nn.init.constant_(m.bias, 0)        # Zero bias for uniform output

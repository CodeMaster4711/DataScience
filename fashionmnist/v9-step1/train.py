...
# ============================================================================
# 1. FIXED RANDOM SEED - Reproduzierbarkeit garantieren
# ============================================================================
...
# ============================================================================
# 2. SIMPLIFY - Keine Data Augmentation (nur ToTensor)
# ============================================================================
...
# ============================================================================
# 3. INIT WELL - Optimierte Gewichtsinitialisierung
# ============================================================================
class MyNN(nn.Module):
    ...
    def _initialize_weights(self):
        """
        Optimierte Gewichtsinitialisierung:
        - He initialization für ReLU layers
        - Bias auf 0
        - Letzte Schicht: spezielle Initialisierung für gleichmäßige Vorhersagen
        """
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                # He initialization für Conv layers (gut für ReLU)
                ...
                # Xavier/Glorot initialization für Linear layers
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                nn.init.constant_(m.bias, 0)

        # Spezielle Initialisierung für die letzte Schicht
        # Damit der initiale Loss bei ~2.302 liegt (-log(1/10))
        # Kleine Gewichte -> gleichmäßigere Vorhersagen am Anfang
...
print("\n" + "="*70)
print("V9 - Optimized CNN with Better Initialization")
print("="*70)
...
print("  ✓ No Data Augmentation (baseline)")
print("  ✓ He/Kaiming initialization for Conv & FC layers")
print("  ✓ Optimized last layer initialization")
print("="*70)
...
# ============================================================================
# 4. VERIFY LOSS @ INIT - Prüfe initialen Loss (~2.302)
# ============================================================================
...
with torch.no_grad():
    ...
    for batch_features, batch_labels in train_loader:
        ...
        # Berechne auch die Durchschnitts-Vorhersage
...
for epoch in range(NUM_EPOCHS):
    ...
    if test_acc > best_acc:
        best_acc = test_acc
        torch.save(model.state_dict(), "best_model_v9.pth")
...
print(f"Improvement vs V8 target (92%): {best_acc - 92:+.2f}%")
print("="*70 + "\n")
...
torch.save(model.state_dict(), "final_model_v9.pth")
...
axes[1].axhline(y=92, color='orange', linestyle='--', label='V8 Target: 92%', alpha=0.7)
axes[1].axhline(y=best_acc, color='darkgreen', linestyle='-', linewidth=2, label=f'V9 Best: {best_acc:.2f}%')
...
plt.tight_layout()
plt.savefig('v9_results.png', dpi=100)
print("\nPlot saved: v9_results.png")
plt.show()

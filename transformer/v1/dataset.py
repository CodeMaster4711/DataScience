...
class DollyDataset(Dataset):
    ...
    def __init__(self, config: TransformerConfig, split: str = "train"):
        ...
        # Load dataset from HuggingFace - Use smaller subset for TINY model
        # Using first 5000 samples instead of full 15k for faster training
        dataset = load_dataset("databricks/databricks-dolly-15k", split="train[:5000]")
        print(f"Using subset of 5000 samples (instead of 15k) for TINY model training")
...
def create_dataloaders(config: TransformerConfig):
    ...
    # Use 0 workers on Mac for TINY model to save memory

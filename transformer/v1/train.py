...
class Trainer:
    def __init__(self, config: TransformerConfig):
        ...
        # Enable gradient checkpointing for memory efficiency (especially on MPS)
        if config.device == "mps" or os.getenv('USE_GRADIENT_CHECKPOINTING', 'true').lower() == 'true':
            print("Enabling gradient checkpointing to save memory (trades compute for memory)")
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            # Calculate perplexity (use unscaled loss)
            perplexity = math.exp(loss.item() * self.config.gradient_accumulation_steps)
